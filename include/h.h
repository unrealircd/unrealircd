...
extern Ban *is_banned(aClient *, aChannel *, int, char **, char **);
...
extern MODVAR int (*place_host_ban)(aClient *sptr, BanAction action, char *reason, long duration);
extern MODVAR int (*run_spamfilter)(aClient *sptr, char *str_in, int type, char *target, int flags, aTKline **rettk);
extern MODVAR int (*join_viruschan)(aClient *sptr, aTKline *tk, int type);
...
extern MODVAR int (*check_banned)(aClient *cptr, int exitflags);
...
extern int add_listmode_ex(Ban **list, aClient *cptr, aChannel *chptr, char *banid, char *setby, time_t seton);
