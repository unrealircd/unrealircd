...
DLLFUNC CMD_FUNC(m_rules)
{
	...
	if (temp == NULL)
	{
		sendto_one(sptr, err_str(ERR_NORULES), me.client.name, parv[0]);
		return 0;

	}

	sendto_one(sptr, rpl_str(RPL_RULESSTART), me.client.name, parv[0], me.client.name);

	while (temp)
	{
		sendto_one(sptr, rpl_str(RPL_RULES), me.client.name, parv[0],
		    temp->line);
		temp = temp->next;
	}
	sendto_one(sptr, rpl_str(RPL_ENDOFRULES), me.client.name, parv[0]);
	return 0;
}
